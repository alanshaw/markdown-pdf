...
function run (filePaths, opts, cb) {
  var tasks = filePaths.map(function (filename) {
    return markdownFileToPdfTask(path.resolve(filename), opts)
  })
 ...
 * @returns {Function}
 */
function markdownFileToPdfTask (filePath, opts) {
  return function (cb) {
    ...
    fs.stat(filePath, function (er, stats) {
      ...
      fs.readFile(filePath, encoding, function (er, data) {
        if (er) return cb(er)
        markdownToPdfTask(data, opts)(cb)
      })
    })
  }
}

function markdownToPdfTask (md, opts) {
  return function (cb) {
    // Convert to HTML
    var html = marked(md)
